...
Independent row spacing, sparse pin grids, module outlines, and literal
silkscreen labels are implemented in footprinter's main branch but are not yet
present in its latest published package. This repo temporarily carries the
matching upstream patch so installs and builds use the correct geometry and
`headermodule` syntax immediately. The patch can be removed after those
features are included in a footprinter release consumed by tscircuit.
